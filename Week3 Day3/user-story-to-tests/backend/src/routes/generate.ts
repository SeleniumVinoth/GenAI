...
generateRouter.post('/', async (req: express.Request, res: express.Response): Promise<void> => {
  try {
    ...
    if (!request.testCategory || !Array.isArray(request.testCategory) || request.testCategory.length === 0) {
      res.status(400).json({ error: 'At least one test category must be selected.' })
      return
    }
