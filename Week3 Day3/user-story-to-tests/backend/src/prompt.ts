...
export function buildPrompt(request: GenerateRequest): string {
  const { storyTitle, acceptanceCriteria, description, additionalInfo, testCategory } = request
  ...
  if (testCategory && Array.isArray(testCategory) && testCategory.length > 0) {
    userPrompt += `\nGenerate ONLY the following test categories: ${testCategory.join(', ')}.`
  } else {
